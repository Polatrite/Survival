...
// upgrade .dm file if called for
	...
	proc/DrawText(text, x, y, width=-1, flags=0, firstline=0, maxlines=-1, icons_x=0, icons_y=0, icon/drawover, list/leftover)
		...
		if(!icout)
			icout = new('blank.dmi')
			icout.setpadding = (icons_x == 1 && icons_y == 1) ? 1 : 0
			icout.setwidth = icons_x + icout.setpadding
			icout.setheight = icons_y
			icout.Crop(icout.setwidth << 5, icout.setheight << 5)
		var/yy=y
		line1=1
		...
		if(sizex>1 || sizey>1)
			for(var/sy=0,sy<sizey,++sy)
				for(var/sx=0,sx<B,sx+=32)
					ic.BlendIcon(new/icon(icon,"[tf] [sx>>5],[sizey-sy-1]"),x+sx,y+(sy<<5),,min(32,B-sx),min(32,height-(sy<<5)))
		else
	...
	proc/QuickName(atom/A, txt, color="#fff", outline, top, size=3, layer=FLY_LAYER)
		...
		var/icon/s = DrawText(txt, x=size*16, y=(top?(32-height-(outline?1:0)):(outline?1:0)), width=size*32,\
	...
	proc/QuickText(atom/A, txt, color="#fff", outline, x=0, y=0, bottom, justify=DF_JUSTIFY_LEFT, layer=FLY_LAYER)
		...
		var/icon/s = DrawText(txt, x=x, y=y, width=w, flags=(justify|DF_NO_FORMAT), icons_x=w/32, icons_y=1)
		if(color=="#fff") color="#ffffff"
		if(outline)
			s.DFP_Outline(color, outline)
		else if(color != "#ffffff")
		...
		var/obj/O = new
		O.layer = layer
		if(justify) O.pixel_x = (1-s.setwidth) * ((justify==DF_JUSTIFY_CENTER) ? 16 : 32)
		for(var/i=0, i<s.setwidth, ++i)
			O.icon = s
			O.icon_state = "[i],0"
			A.overlays += O
...
icon
	...
	proc/DFP_Dilate()
		 ...
		 */
		if(world.byond_version < 430 && setwidth != setheight)
			var/s = max(setwidth + setpadding, setheight) * 32
			Crop(s, s)
		var/icon/I = new(src)
		for(var/dir=8, dir, dir>>=1)
			...
			Blend(J, ICON_OR)
		if(world.byond_version < 430 && setwidth != setheight)
			Crop((setwidth + setpadding) * 32, setheight * 32)
	...
	proc/BlendIcon(icon/c,x,y,operation=ICON_OR,iw=32,ih=32)
		...
		var/idx
		var/oy=y&31
		for(iy=max(0,y)>>5,(oy>-ih && iy<setheight),++iy)
			var/fx=max(0,x)>>5
			var/ox=x-(fx<<5)
