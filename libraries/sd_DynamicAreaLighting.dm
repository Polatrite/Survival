...
DAY/NIGHT CYCLES
	...
	all outside areas. light_level should be a value from 0 to 4,
	where 0 is full darkness and 4 is full daylight.
	...
	proc
		...
		sd_SetOpacity(new_opacity as num)
			...
			for(A in range(sd_top_luminosity,src))
				if(!(src in view(A.luminosity,A))) continue
				var/turfflag = isturf(A)
				if(A.luminosity && get_dist(A,src)<=A.luminosity+turfflag)
					affected[A] = view(A.luminosity, A)
				if(sd_light_outside && (A in sd_light_spill_turfs))
					if(!spill) spill=new
			...
			if(opacity)
				for(A in affected)
					//affected[A] -= view(A.luminosity, A)
					for(T in affected[A]-view(A.luminosity, A))
						T.sd_lumcount -= (A.luminosity-get_dist(A,T))
						T.sd_LumUpdate()
				for(A in spill)
					if(A.opacity && A!=src) continue
					//spill[A] -= view(sd_light_outside, A)
					for(T in (A==src)?spill[A]:(spill[A]-view(sd_light_outside,A)))
				...
				for(A in affected)
					for(T in view(A.luminosity, A) - affected[A])
						T.sd_lumcount += (A.luminosity-get_dist(A,T))
						T.sd_LumUpdate()
				for(A in spill)
					if(A.opacity) continue
					for(T in (A==src)?spill[A]:(view(sd_light_outside, A)-spill[A]))
						if(T.loc:sd_outside) continue
						T.sd_lumcount += (sd_light_outside-get_dist(A,T))
						T.sd_LumUpdate()
...
turf
	...
	proc
		...
		sd_LumUpdate()
			...
			var/light = min(max(sd_lumcount,0),4)
...
area
	...
	proc
		sd_LightLevel(level = sd_light_level as num, keep = 1)
			...
			level = min(max(level,0),4)	// restrict range
